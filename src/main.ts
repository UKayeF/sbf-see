...
import { QuizConfig, defaultQuizConfig, createQuizConfig } from './models/quizConfig';
  ...
  if (config.title) {
    ...
    for (const cat in data.categories) {
      if (config.title === 'Fragen mit Bildern') {
        allQuestions.push(...data.categories[cat].filter(q => q.image && q.image.length > 0));
      } else if (config.title === 'Fragen mit Antwort-Bildern') {
        allQuestions.push(...data.categories[cat].filter(q => q.answers.some(a => a.images && a.images.length > 0)));
      }
    }
    const selected = selectQuestions(allQuestions, categoryConfig.questionsCount);
...
function renderApp() {
  ...
  const config = quizType === 'default' ? defaultQuizConfig : createQuizConfig(quizType);
    ...
    if (state.showFeedback) {
      ...
      document.querySelectorAll('.answer-btn').forEach(btn => {
        btn.addEventListener('click', () => {
          const selectedIndex = parseInt((btn as HTMLElement).dataset.index || '0');
